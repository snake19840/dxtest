...
@WebServlet({"/out_contractMain"})
public class out_contractMain
  extends HttpServlet
{
...
//    		+ "t5.myid=t6.mypayid and t2.realpay>='" + DataSets.anytomorrow(startDate) + "'  and t2.realpay<'" + DataSets.anytomorrow(endDate) + "' order by now_t2";
    String sql ="select t1.ID AS ID_t1,t1.CONTRACTID AS CONTRACTID_t1,t1.CONTRACTNAME AS CONTRACTNAME_t1, t1.CONTRACTNOWDATE "
    		...
    		+ "and t1.myid=t5.myid and  t3.oppid=t4.opppayid  and  t5.myid=t6.mypayid and t1.MARKDATE>='"+DataSets.anytomorrow(startDate)+"' "
    		+ "and t1.MARKDATE<'"+DataSets.anytomorrow(endDate)+"'  order by MARKDATE_t1";
    
    
    List<Equ> list = SelectAll.Warningstatu(sql);
    List<Equ> contractList=ContractDao.contractListMain(DataSets.anytomorrow(startDate), DataSets.anytomorrow(endDate));
