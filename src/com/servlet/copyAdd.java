...
/**
 * Servlet implementation class copyAdd
 */
@WebServlet("/copyAdd")
public class copyAdd extends HttpServlet {
...
//		System.out.println(myid);
		...
		if (count>0) {
			out.print("{\"statu\":0,\"WarningGsons\":\" 已经有相同的合同号 \"}");
			out.close();
		}else {
...
//			System.out.println("endmy:"+myid);
//			System.out.println("endopp:"+oppid);
			sql="insert into DXTESTCONTRACT (contractid,	contractname,	oppunit,	markdate,	contractnlife,	statu,	camount,	plan1,	plandate1,	"
					+ "plan2,	plandate2,	plan3,plan4,	planm2,	sgmoney,	myname,	oppname,	oppid,	myid,	classson,sgmoney) values('"+contractid+"',	'"+contractname+"',	"
							+ "'"+oppunit+"',	'"+markdate+"',	'"+contractnlife+"',	'"+statu+"',	'"+camount+"',	'"+plan1+"',	'"+plandate1+"',	'"+plan2+"',	"
									+ "'"+plandate2+"',	'"+plan3+"','"+plan4+"',	'"+planm2+"',	'"+sgmoney+"',	'"+myname+"',	'"+oppname+"',	'"+oppid+"',	'"+myid+"',	'"+classson+"','"+sgmoney+"')";
			flag=SelectAll.insertItem(sql);
			out.print("{\"statu\":1,\"WarningGsons\":\" 已经有相同的合同号 \"}");
			out.close();
			
		}
		
		 
//		boolean flag1 =SelectAll.insertItem(sql);
